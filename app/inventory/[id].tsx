...
export default function InventoryDetailScreen() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      ...
      setLoading(true);
      setError(null);
      const res = await api.getInventoryDetails(id);
      if (active) {
        if (res.success && res.data) {
          setItem(res.data);
        } else {
          const localMatch = inventory.find((entry) => entry.id === id);
          if (localMatch) {
            setItem(localMatch);
          } else {
            setError(res.message || 'Component not found');
          }
        }
        setLoading(false);
      }
    ...
    return (
      <View style={[uiStyles.screen, { backgroundColor: colors.background }]}>
        <View style={[uiStyles.scroll, { paddingTop: insets.top + 4 }]}>
          <ScreenHeader title="Inventory Detail" subtitle="Loading..." onBack={() => router.back()} />
          <DetailsSkeleton />
        </View>
      </View>
    );
    ...
    return (
      <View style={[styles.notFound, { backgroundColor: colors.background }]}>
        ...
        <Text style={{ color: colors.mutedForeground, textAlign: 'center', marginHorizontal: 24 }}>
          {error || 'The requested inventory component could not be loaded.'}
        </Text>
  ...
  const ratio = Math.min(100, (item.quantity / (item.maxStock || 1)) * 100);

  const openAdjust = (nextMode: 'add' | 'remove') => {
    setMode(nextMode);
    setModalOpen(true);
  };
  ...
  const saveAdjustment = async () => {
    ...
    setUpdating(true);
    const adjustmentVal = mode === 'add' ? value : -value;
    const success = await adjustInventory(item.id, adjustmentVal);
    setUpdating(false);
    ...
    if (success) {
      setItem((prev) =>
        prev
          ? {
              ...prev,
              quantity: Math.max(0, prev.quantity + adjustmentVal),
    ...
    <View style={[uiStyles.screen, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={[uiStyles.scroll, { paddingTop: insets.top + 4 }]}>
        ...
        <View style={[styles.titleCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={[styles.componentIcon, { backgroundColor: colors.secondary }]}>
            <Feather name="cpu" size={25} color={colors.primary} />
          </View>
        ...
        <View style={[styles.stockCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.stockHeader}>
            <View>
              ...
              <Text style={[styles.stockValue, { color: colors.foreground }]}>
                {item.quantity} <Text style={styles.stockMax}>/ {item.maxStock}</Text>
              </Text>
            </View>
            <View
              style={[
          ...
          <ProgressBar value={ratio} color={item.quantity <= item.threshold ? '#e2a14b' : colors.primary} />
          <Text style={styles.threshold}>Minimum threshold: {item.threshold} units</Text>
        </View>

        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Component information</Text>
        ...
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Inventory actions</Text>
        <View style={styles.actions}>
          <View style={styles.actionHalf}>
            <PrimaryButton title="Add stock" icon="plus" onPress={() => openAdjust('add')} />
          </View>
          <View style={styles.actionHalf}>
            <Pressable
              onPress={() => openAdjust('remove')}
              style={[styles.secondaryButton, { backgroundColor: colors.card, borderColor: colors.border }]}
            >
              <Feather name="minus" size={18} color={colors.destructive} />
              <Text style={[styles.secondaryText, { color: colors.destructive }]}>Remove stock</Text>
            </Pressable>
          </View>
        </View>
      </ScrollView>
        ...
        <View style={styles.modalBackdrop}>
          <View style={[styles.sheet, { backgroundColor: colors.card, paddingBottom: insets.bottom + 18 }]}>
            ...
            <Text style={styles.sheetSubtitle}>
              {mode === 'add' ? 'Record incoming components.' : 'Record components used or removed.'}
            </Text>
            ...
            <TextInput
              ...
              style={[styles.amountInput, { borderColor: colors.input, color: colors.foreground }]}
            />

            <View style={styles.sheetActions}>
              <Pressable onPress={() => setModalOpen(false)} style={styles.cancelButton} disabled={updating}>
                <Text style={styles.cancelText}>Cancel</Text>
              </Pressable>
              <View style={styles.saveButton}>
                <PrimaryButton title={updating ? 'Saving…' : 'Save adjustment'} onPress={saveAdjustment} disabled={updating} />
              </View>
            </View>
          </View>
        </View>
