...
export function JobCard({ job, onPress }: { job: Job; onPress: () => void }) {
  const colors = useColors();
  const totalPcs = job.quantity ?? 0;
  const doneQty = job.finalQty ?? totalPcs;
  const failQty = job.failedQty ?? 0;
  const pendQty = job.pendingQty ?? 0;
  ...
  return <Pressable onPress={onPress} style={({ pressed }) => [styles.jobCard, { borderColor: colors.border }, pressed && { transform: [{ scale: 0.985 }], opacity: 0.9 }]}>
    ...
    <View style={styles.qtyRow}>
      <Text style={styles.qtyTotalText}>{totalPcs} Pcs</Text>
      <Text style={styles.qtyDot}> · </Text>
      <Text style={styles.qtyDoneText}>{doneQty} Done</Text>
      <Text style={styles.qtySlash}> / </Text>
      <Text style={styles.qtyFailText}>{failQty} Fail</Text>
      <Text style={styles.qtySlash}> / </Text>
      <Text style={styles.qtyPendText}>{pendQty} Pend</Text>
    </View>
...
const styles = StyleSheet.create({
  ...
  logoCompact: { width: 160, height: 44 },
  ...
  qtyRow: { flexDirection: 'row', alignItems: 'center', marginTop: 10, flexWrap: 'wrap' },
  qtyTotalText: { fontSize: 14, fontWeight: '800', color: '#173524' },
  qtyDoneText: { fontSize: 13, fontWeight: '700', color: '#10b981' },
  qtyFailText: { fontSize: 13, fontWeight: '700', color: '#ef4444' },
  qtyPendText: { fontSize: 13, fontWeight: '700', color: '#f59e0b' },
  qtyDot: { fontSize: 14, fontWeight: '700', color: '#8b978d' },
  qtySlash: { fontSize: 13, fontWeight: '600', color: '#9ca3af' },
  jobMeta: { flexDirection: 'row', gap: 18, borderTopWidth: 1, borderBottomWidth: 1, borderColor: '#edf2ed', paddingVertical: 13, marginTop: 12 },
